import { musicaDjColaAgregar } from "@/lib/musica-modo-dj";
...
export async function POST(request: Request) {
  ...
  const ok = await musicaDjColaAgregar(db, eventoId, aporteId);
  if (!ok) {
    return NextResponse.json({ ok: false, message: "No se pudo agregar a la cola." }, { status: 400 });
  }
