import { musicaDjColaQuitar } from "@/lib/musica-modo-dj";
...
export async function POST(request: Request) {
  ...
  const ok = await musicaDjColaQuitar(db, eventoId, aporteId);
  if (!ok) {
    return NextResponse.json({ ok: false, message: "No se pudo quitar de la cola." }, { status: 400 });
  }
