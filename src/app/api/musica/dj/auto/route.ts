import { musicaDjToggleAuto } from "@/lib/musica-modo-dj";
...
export async function POST(request: Request) {
  ...
  const ok = await musicaDjToggleAuto(db, eventoId, activo);
  if (!ok) {
    return NextResponse.json({ ok: false, message: "No se pudo actualizar Auto DJ." }, { status: 500 });
  }
