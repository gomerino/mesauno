import { musicaDjPausar } from "@/lib/musica-modo-dj";
...
export async function POST(request: Request) {
  ...
  const ok = await musicaDjPausar(db, eventoId);
  if (!ok) {
    return NextResponse.json({ ok: false, message: "No se pudo pausar." }, { status: 500 });
  }
