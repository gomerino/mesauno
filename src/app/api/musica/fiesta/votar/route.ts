...
import { musicaFiestaRegistrarVotoLive } from "@/lib/musica-fiesta-en-vivo";
...
export async function POST(request: Request) {
  ...
  if (token) {
    ...
    const res = await musicaFiestaRegistrarVotoLive(db, {
      evento_id: eventoId,
      aporte_id: aporteId,
    ...
    if (!res.ok) {
      if (res.code === "fiesta_inactiva") {
        return NextResponse.json({ code: res.code, message: "El modo fiesta no está activo." }, { status: 400 });
      }
      if (res.code === "no_aprobado") {
        return NextResponse.json({ code: res.code, message: "Solo puedes votar canciones aprobadas." }, { status: 400 });
      }
      return NextResponse.json({ code: res.code, message: "No se pudo registrar el voto." }, { status: 400 });
    }
    return NextResponse.json({
      ok: true,
      votos_live: res.votos_live,
