...
import { musicaFiestaActivar } from "@/lib/musica-fiesta-en-vivo";
...
export async function POST(request: Request) {
  ...
  const ok = await musicaFiestaActivar(db, eventoId);
  if (!ok) {
    return NextResponse.json({ code: "db", message: "No se pudo activar el modo fiesta." }, { status: 500 });
  }
