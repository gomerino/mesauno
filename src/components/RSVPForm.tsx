...
export function RSVPForm({ invitadoId, initialRestricciones, initialEstado, compact = false }: Props) {
  ...
  /** En lg, 2 columnas arriba (asistencia + restricciones); fondo de millas ocupa todo el ancho abajo. */
  const gridGap = compact ? "gap-2 lg:grid-cols-2 lg:gap-3" : "gap-3 lg:grid-cols-2 lg:gap-4";
    ...
    <form onSubmit={onSubmit} className={`flex flex-col ${compact ? "gap-2" : "gap-3"}`}>
      <div className={`grid grid-cols-1 ${gridGap}`}>
        ...
        <div className={`${card} min-w-0 lg:col-span-2`}>
          ...
          <p className="mt-0.5 max-w-3xl text-xs leading-relaxed text-gray-600 sm:text-[11px] sm:leading-snug">
          ...
          <div className="mt-2.5 min-w-0 overflow-hidden rounded-lg border border-[#001d66]/20 bg-[#001d66]/[0.06] px-3 py-2 sm:px-3 sm:py-2.5 lg:px-4 lg:py-3">
            <dl className="divide-y divide-[#001d66]/15 lg:grid lg:grid-cols-2 lg:gap-x-10 lg:gap-y-0 lg:divide-y-0 xl:grid-cols-4 xl:gap-x-6">
              <div className="flex flex-col gap-1 py-2.5 first:pt-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:py-2 lg:flex-col lg:gap-1 lg:rounded-lg lg:bg-white/40 lg:px-3 lg:py-2.5 lg:first:pt-2.5 lg:last:pb-2.5">
                <dt className="shrink-0 text-[10px] font-semibold uppercase tracking-wide text-gray-500">
                  Cuenta corriente
                </dt>
                <dd className="min-w-0 font-mono text-sm font-semibold tabular-nums tracking-wide text-gray-900 sm:max-w-[58%] sm:text-right sm:text-[13px] lg:max-w-none lg:text-left">
                  53-17217-05
                </dd>
              </div>
              <div className="flex flex-col gap-1 py-2.5 first:pt-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:py-2 lg:flex-col lg:gap-1 lg:rounded-lg lg:bg-white/40 lg:px-3 lg:py-2.5 lg:first:pt-2.5 lg:last:pb-2.5">
                ...
                <dd className="min-w-0 text-sm font-medium leading-snug text-gray-900 sm:max-w-[58%] sm:text-right sm:text-[13px] lg:max-w-none lg:text-left">
                  Banco de Chile
                </dd>
              </div>
              <div className="flex flex-col gap-1 py-2.5 first:pt-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:py-2 lg:flex-col lg:gap-1 lg:rounded-lg lg:bg-white/40 lg:px-3 lg:py-2.5 lg:first:pt-2.5 lg:last:pb-2.5">
                ...
                <dd className="min-w-0 font-mono text-sm font-semibold tabular-nums text-gray-900 sm:max-w-[58%] sm:text-right sm:text-[13px] lg:max-w-none lg:text-left">
                  13.468.424-0
                </dd>
              </div>
              <div className="flex flex-col gap-1 py-2.5 first:pt-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:py-2 lg:flex-col lg:gap-1 lg:rounded-lg lg:bg-white/40 lg:px-3 lg:py-2.5 lg:first:pt-2.5 lg:last:pb-2.5">
                ...
                <dd className="min-w-0 break-words text-sm font-medium leading-snug text-gray-900 sm:max-w-[58%] sm:text-right sm:text-[13px] lg:max-w-none lg:text-left">
