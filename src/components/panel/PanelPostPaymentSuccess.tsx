  ...
  return (
    <div className="mx-auto max-w-lg space-y-8 pb-8 md:max-w-2xl md:pb-12">
      ...
      <header className="motion-safe:animate-postPayReveal motion-safe:[animation-delay:40ms] motion-reduce:animate-none motion-reduce:opacity-100">
        <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-invite-gold/90">Listo</p>
      ...
      <section className="motion-safe:animate-postPayReveal motion-safe:[animation-delay:160ms] motion-reduce:animate-none motion-reduce:opacity-100 rounded-xl border border-invite-gold/20 bg-gradient-to-br from-[#0c101c] to-[#0a0e16] p-5 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.5)]">
      ...
      <div className="motion-safe:animate-postPayReveal motion-safe:[animation-delay:220ms] motion-reduce:animate-none motion-reduce:opacity-100">
        <Link
          href={primaryHref}
          className={panelCtaJurnexPrimary + " min-h-[52px] w-full justify-center px-6"}
        >
          {primaryLabel}
        </Link>
      </div>
      ...
      <div className="motion-safe:animate-postPayReveal motion-safe:[animation-delay:280ms] motion-reduce:animate-none motion-reduce:opacity-100 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {secondaries.map((s) => (
          <Link
            key={s.label}
            href={s.href}
            className="flex min-h-[48px] items-center justify-center gap-2 rounded-xl border border-white/[0.07] bg-white/[0.03] px-3 py-2.5 text-center text-xs font-medium text-slate-300 transition hover:border-invite-gold/25 hover:bg-white/[0.05] hover:text-white"
          >
