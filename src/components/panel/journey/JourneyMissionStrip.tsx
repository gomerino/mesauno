    ...
    >
      {hideMisionesLabel ? null : (
        ...
        >
          ...
          <span className="tabular-nums text-[10px] font-medium text-teal-200/85 md:text-[11px]">
          ...
          >
            <span
              className={`flex items-center justify-center rounded-full border text-[8px] font-semibold transition-colors ${c({
                normal: "h-6 w-6 sm:h-7 sm:w-7 sm:text-[9px] md:h-8 md:w-8 md:text-[10px]",
                small: "h-5 w-5 sm:h-6 sm:w-6 sm:text-[8px] md:h-7 md:w-7 md:text-[9px]",
              })} ${
                s.state === "done"
            ...
            >
              {s.state === "locked" ? (
                <Lock className="h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4" strokeWidth={2} aria-hidden />
              ) : s.state === "done" ? (
                "✓"
              ) : s.state === "active" ? (
                <span className="h-1.5 w-1.5 rounded-full bg-[#F5C451] sm:h-2 sm:w-2 md:h-2.5 md:w-2.5" aria-hidden />
              ) : (
                <span className="text-[9px] tabular-nums opacity-80 sm:text-[10px] md:text-[11px]">{i + 1}</span>
              )}
            </span>
            <span
              className={
                c({
                  normal:
                    "max-w-full truncate text-center text-[8px] font-medium leading-tight text-slate-500 sm:text-[8px] md:text-[10px] lg:text-[11px]",
                  small:
                    "max-w-full truncate text-center text-[7px] font-medium leading-tight text-slate-500 sm:text-[8px] md:text-[10px] lg:text-[11px]",
                })
              }
            >
              {s.label}
